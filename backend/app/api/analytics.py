...
async def get_dashboard_analytics(
    ...
    try:
        ...
        analytics = service.get_enterprise_metrics(hr_id=current_user.id if current_user.role != "super_admin" else None)
        
        # 2. Application Status Distribution (for Charts)
        # Performance Optimization: Use load_only and direct count
        ...
        if current_user.role != "super_admin":
            status_query = status_query.filter(or_(Application.hr_id == current_user.id, Job.hr_id == current_user.id))
        ...
        if current_user.role != "super_admin":
            activity_query = activity_query.filter(or_(Application.hr_id == current_user.id, Job.hr_id == current_user.id))
        ...
        logger.critical(f"CRITICAL ERROR in get_dashboard_analytics: {e}")
...
async def get_interview_reports(
    ...
    try:
        ...
        # Pre-fetch all questions and answers for all reports in bulk to avoid N+1
        interview_ids = [r.interview_id for r in db_reports if r.interview_id]
        all_questions_map = {}
        all_answers_map = {}
        ...
        if interview_ids:
            ...
            for q in questions:
                if q.interview_id not in all_questions_map:
                    all_questions_map[q.interview_id] = []
                all_questions_map[q.interview_id].append(q)
