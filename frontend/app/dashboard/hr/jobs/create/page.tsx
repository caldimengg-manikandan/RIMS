...
import { Sparkles, UploadCloud, Loader2, FileText, X, PlusCircle, ArrowLeft, CheckCircle2, BookOpen, Link } from 'lucide-react'
...
export default function HRCreateJobPage() {
    ...
    useEffect(() => {
        const handleBeforeUnload = (e: BeforeUnloadEvent) => {
            const hasData = formData.title || formData.description || formData.location || 
                          formData.aptitude_enabled || formData.uploaded_question_file;
    ...
    // JP004/H012-H016: non-blocking real-time validation (title/description)
    ...
    const validateTitle = (value: string) => {
        ...
        if (v.length < 3 || v.length > 100) { setTitleError('Job title must contain meaningful text with alphabets'); return }
        if (!/[A-Za-z]/.test(v)) { setTitleError('Job title must contain meaningful text with alphabets'); return }
        if (!/^[A-Za-z0-9\s\-.,()\/]+$/.test(v)) { setTitleError('Job title must contain meaningful text with alphabets'); return }
        if (/([\-.,()\/])\1{3,}/.test(v)) { setTitleError('Job title must contain meaningful text with alphabets'); return }
        setTitleError(null)
    }
    ...
    const validateDescription = (value: string) => {
        ...
        if (v.length < 10) { setDescError('Description must contain meaningful text (minimum 10 characters)'); return }
        if (!/[A-Za-z]/.test(v)) { setDescError('Description must contain meaningful text'); return }
        if (/([\-.,()\/])\1{3,}/.test(v)) { setDescError('Description must contain meaningful text'); return }
        setDescError(null)
    }

    const validateDuration = (value: number) => {
        if (!value) { setDurationError(null); return }
        if (value < 1 || value > 300) { setDurationError('Interview duration must be between 1 and 300 minutes'); return }
