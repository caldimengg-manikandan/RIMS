...
export function BatchUploadModal({ isOpen, onClose, onSuccess }: BatchUploadModalProps) {
  ...
  useEffect(() => {
    ...
    const checkCompletion = async () => {
      ...
      try {
        const data = await APIClient.get(`/api/applications?job_id=${selectedJobId}`)
        ...
        setPollingStatus(`Finalizing data: ${completedCount}/${batchApps.length} candidates ready...`)

        if (completedCount === batchApps.length) {
          setIsExportReady(true)
          setFinalBatchData(batchApps)
        }
      } catch (error) {
        console.error("Error polling applications state:", error)
      }
    }
